...
def process_plan(plan_num, working_directory, output_path, state, data, num_districts):
    ...
    logging.debug(f"Updaters initialized: {updaters.keys()}")
    ...
    logging.debug(f"New assignment for plan {plan_num} created.")
    logging.info(f"Finished new assignment for plan {plan_num}.")
    ...
    logging.debug(f"Initial partition created for plan {plan_num}.")
    ...
    for district in final_partition["democrats"].keys():
        ...
        winner = "democrats" if dem_count > rep_count else "republicans"
        final_results["winner"].append(winner)
        ...
        area = final_partition["arealand"].get(district, 1)
        density = (total_population / area) * 1000 if area > 0 else 0
        region_category = "Rural" if density < 0.1 else "Urban" if density > 3 else "Suburban"

        final_results["category"].append(region_category)

        total_pop = total_population if total_population > 0 else 1  
        final_results["black_pct"].append(black_count / total_pop)
        final_results["white_pct"].append(white_count / total_pop)
        final_results["asian_pct"].append(asian_count / total_pop)
        final_results["hispanic_pct"].append(hispanic_count / total_pop)
        final_results["aindalk_pct"].append(aindalk_count / total_pop)
        final_results["hipi_pct"].append(hipi_count / total_pop)
        final_results["other_pct"].append(other_count / total_pop)
        final_results["twoormore_pct"].append(twoormore_count / total_pop)
        final_results["total_population"].append(total_population)

        precinct_geometries = [shape(graph.nodes[precinct]["geometry"]) for precinct in final_partition.assignment if final_partition.assignment[precinct] == district]
        combined_geometry = unary_union(precinct_geometries) if precinct_geometries else None
    ...
    return final_results, plan_summary
...
def seawulf(working_directory, output_path, state, data, num_plans, num_districts):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            plan_results, plan_summary = future.result()
            all_plan_results.append(plan_results)
            all_plan_summaries.append(plan_summary)
            
    all_results_df = pd.concat([pd.DataFrame(results) for results in all_plan_results], keys=range(num_plans), names=["plan_num", "index"])
    all_results_gdf = gpd.GeoDataFrame(all_results_df, geometry="geometry", crs="EPSG:4326")
    all_results_gdf.to_file(output_path + f"{state}_district_plans_1.geojson", driver="GeoJSON")
    logging.info("All results saved as GeoJSON.")

    print(all_plan_summaries)
    all_plan_summary_df = pd.concat(
        [pd.DataFrame([summary]) for summary in all_plan_summaries], 
        keys=range(len(all_plan_summaries)), 
        names=["plan_num", "index"]
    )
    all_plan_summary_df.to_json(output_path + f"{state}_ensemble_summary_1.json", orient="records", lines=True)
    logging.info("Summary data saved.")
...
if __name__ == "__main__":
    ...
    seawulf(current_directory, os.path.join(current_directory, ""), "CA", california_data, 10, 52)
